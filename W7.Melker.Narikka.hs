...
countRange low high is = go 0 is
  where
    go c []     = c
    go c (i:is)
      | i >= low && i <= high = go (c + 1) is
      | otherwise             = go c is
...
chess h w = c h False
  where
    knot = '#' : '.' : knot
    buildRow flippin = take w $ drop (fromEnum flippin) knot
    c h flippin
      | h <= 0    = ""
      | otherwise = buildRow flippin ++ "\n" ++ c (h-1) (not flippin)
...
sumEithers = go Nothing
  where
    go s        []             = s
    go s        ((Left  _):xs) = go s xs
    go Nothing  ((Right x):xs) = go (Just x) xs
    go (Just s) ((Right x):xs) = go (Just (s + x)) xs
...
instance Functor Twos where
  ...
  fmap f (Continue a b n) = Continue (f a) (f b) (fmap f n)
...
-- the passthrough looks silly here either way :/
--step x | even x    = get >>= (\s -> put (s + x))
--       | otherwise = modify (\x -> x)
--       | otherwise = get >>= (\s -> put s)

-- hey, this looks less silly
step x = when (even x) (get >>= (\s -> put (s + x)))
...
instance Functor Env where
  fmap f (MkEnv e) = MkEnv (\s -> f (e s))

instance Monad Env where
  e >>= f = MkEnv res
    where
      res s0 = let a   = runEnv e s0
                   op2 = f a
               in  runEnv op2 s0
  return = \x -> MkEnv (\_ -> x)
