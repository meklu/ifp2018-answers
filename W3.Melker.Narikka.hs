...
data UpDown = UpDownIncr Int | UpDownDecr Int
  deriving Show
...
tick :: UpDown -> UpDown
tick (UpDownIncr c) = UpDownIncr $ c + 1
tick (UpDownDecr c) = UpDownDecr $ c - 1

-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown
toggle (UpDownIncr c) = UpDownDecr c
toggle (UpDownDecr c) = UpDownIncr c
...
-- cases look a lot cleaner parameter-wise than whatever I was doing above :)
insertL x t = case t of
  Leaf       -> Node x Leaf Leaf
  Node v l r -> Node v (insertL x l) r
...
measure t = case t of
  ...
  Node _ l r -> Node (treeSize t) (measure l) (measure r)
...
-- sumf and lengthf appropriately.
...
foldTree f x t = case t of
  Leaf         -> x
  (Node v l r) -> f v (foldTree f x l) (foldTree f x r)
...
rgb Blue  = [0,0,1]
rgb col = case col of
  Mix    a b -> go (rgb a) (rgb b)
    where
      go [] []         = []
      go (x:xs) (y:ys) = min 1 (x + y) : go xs ys
  Darken f c -> map (*(1.0-f)) (rgb c)
